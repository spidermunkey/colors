...
function $$(arg,element = document) {
    return Array.from(element.querySelectorAll(arg));;
}
function log() {
...
function trapInput(e,cb) {
    const {key} = e;
    if (key === 'Enter' && (nameChange !== '' || nameChange !== undefined))
        cb();
...
function tag(elementName, props, ...children) {
    ...
    if (props.classList)
        result.classList.add(...props.classList)

    for (const attr in props.attributes)
    result.setAttribute(attr, props.attributes[attr])
...
function toClipboard(value,message) {
    ...
    if (message) console.log('copied to clipboard',value,message)
}
...
function copyCurrentHex() {
    if (app.color) window.navigator.clipboard.writeText(app.color.hex)
    else console.warn('no current color',app.color)
}
...
function trapInput(e) {
    if (!appState.color) return
    const name = appState.color.name;
    const {key} = e;
    if (key === 'Enter' && (name !== '' || name !== undefined))
        submitInput();
    // if (key === 'Backspace' && $('.inp input').value === $('.color-content .color-title .label').textContent)
    //     $('.inp input').value = '';
}
...
function submitInput() {
    // update in db ?
    if (appState.nameChange)
    appState.color.name = appState.nameChange;
