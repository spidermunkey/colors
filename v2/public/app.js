...
import { each } from "./lib/javascript/dom.js";
const PORT = 1280
...
$('#app').addEventListener('click', (event) => {
    ...
    if (!clickedColorEditor && colorPreview.cphActive == true) {
        colorPreview.setCPHModeText();
    }
})
...
(function getDefaultColorsForHomePage() {
    ...
    async function getDefaultHueAndPaint(hue) {
        const {data} = await axios.get(`http://localhost:${PORT}/colors/crayolaExtended?hue=${hue}`);
...
function createPalette(name) {
    const palette = new Dashboard(`http://localhost:${PORT}/colors/${name}`, name , 'palette');
    palettes[name] = palette
}

function createCollection(name) {
    collections[name] = new Dashboard(`http://localhost:${PORT}/colors/collections/${name}`,name, 'collection');
}
