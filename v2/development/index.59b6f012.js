...
const api = {
    routes: {
        async add (collectionName, colorName, colorHex) {
            const response = await axios.post(`http://localhost:${PORT}/colors/add/${collectionName}`, {
                colorName,
                colorHex
            });
            return response;
        ...
        async create (collectionName, desc) {
            const response = await axios.post(`http://localhost:${PORT}/colors/collection/create`, {
        ...
        async delete (name) {
            const response = await axios.delete(`http://localhost:${PORT}/colors/collection`, {
                data: {
                    cName: name
                }
            }).catch(function(e) {
    ...
    async listPalettes () {
        const { data  } = await axios.get(`http://localhost:${PORT}/colors/meta`);
...
//# sourceMappingURL=index.59b6f012.js.map
